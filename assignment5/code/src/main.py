...
def main() -> None:
    ...
    input_tensor_train, target_tensor_train, input_language, target_language, max_length_targ, max_length_inp, input_vals, target_vals = read_data(
        dataset_size)
    ...
    embedding_dim = 256
    units = 1024
    vocab_inp_size = len(input_language.word_index) + 1
    vocab_tar_size = len(target_language.word_index) + 1
    ...
    with strategy.scope():
        ...
        run_train(input_tensor_train, target_tensor_train, target_language, checkpoint, checkpoint_dir,
                  encoder, optimizer, decoder, steps_per_epoch, BUFFER_SIZE, BATCH_SIZE)

    # restoring the latest checkpoint in checkpoint_dir
    checkpoint.restore(tf.train.latest_checkpoint(checkpoint_dir))

    # run tests and get score
    run_tests(max_length_targ, max_length_inp, input_language, target_language,
